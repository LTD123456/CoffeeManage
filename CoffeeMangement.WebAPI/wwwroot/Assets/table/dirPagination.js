 ...
 */
(function () {
    angular.module('dirPaginationDirective', [])
        ...
        .directive('dirPaginationControls', ['paginationService', function (paginationService) {
            ...
            return {
                ...
                templateUrl: function(elem, attrs) {
                    return attrs.templateUrl || '/Scripts/posim/directive/dirPagination.tpl.html';
                },
                scope: {
                    maxSize: '=?',
                    onPageChange: '&?'
                ...
                link: function(scope, element, attrs) {
                    ...
                    //var pageNumber = calculatePageNumber(i, currentPage, paginationRange, totalPages);
                    //Update set rowperpage 
                    //6/6/2016
                    scope.count = ( scope.$parent.pageOptions != undefined && scope.$parent.pageOptions.limit != undefined ) ? scope.$parent.pageOptions.limit : scope.counts[0];
                    //end set
                    ...
                    scope.setCurrent = function(num) {
                        if (numberRegex.test(num)) {
                            if (0 < num && num <= scope.pagination.last) {
                                ...
                                scope.count = paginationService.getItemsPerPage(paginationId);
                                // if a callback has been set, then call it with the page number as an argument
                                if (scope.onPageChange) {
                                    scope.onPageChange({ newPageNumber: num, newPageSize: paginationService.getItemsPerPage(paginationId) });
                                }
                            }
                        }
                    };
                    ...
                    scope.setItemsPerPage = function (numSize)
                    {
                        scope.pages = generatePagesArray(1, paginationService.getCollectionLength(paginationId), numSize, paginationRange);
                        scope.pagination.current = 1; // parseInt(paginationService.getCurrentPage(paginationId));
                        scope.pagination.last = scope.pages[scope.pages.length - 1];

                        paginationService.setItemsPerPage(paginationId, numSize);
        ...
        .service('paginationService', function () {
            ...
            this.setItemsPerPage = function (instanceId, val) {
                instances[instanceId].itemsPerPage =  val == undefined ? 10 : val;
            };
