(function (angular, factory) {
        ...
        function ($compile, $q, $parse) {
            ...
            return {
                ...
                compile: function (element) {
                    ...
                    // custom header
                    var thead = element.find('thead[data-thead="true"]');
                    ...
                    return function (scope, element, attrs) {
                        ...
                        if (!element.hasClass('ng-table')) {
                            ...
                            element.find('thead[data-thead="true"]').remove();
        ...
        function ($compile) {
            ...
            return {
                ...
                link: function (scope, element, attrs) {
                    ...
                    //console.log(scope.params.data.length);

                    scope.params.settings().$scope.$on('ngTableAfterReloadData', function () {
                        scope.pages = scope.params.generatePagesArray(scope.params.page(), scope.params.total(), scope.params.count());
                        scope.curPage = scope.params.page();

                        scope.lastpage = scope.pages.length > 0 ?  _.filter(scope.pages, function (page) { return page.type == "last" }).length > 0 ? _.filter(scope.pages, function (page) { return page.type == "last" })[0].number : 1 : 1;
                        scope.changePage = function (page) {
                            console.log(page);
                            if (page !== null && page !== undefined && page != '') {
                                if (parseInt(page) > scope.lastpage) {
                                    page = scope.lastpage;
                                    scope.curPage = page;
                                    if (angular.element('[ng-model="curPage"]').length > 0) {
                                        angular.element('[ng-model="curPage"]').val(page);
                                    }
                                }
    ...
    angular.module('ngTable').run(['$templateCache', function ($templateCache) {
        ...
        $templateCache.put('ng-table/header.html', '<tr> <th ng-repeat="column in $columns" ng-class="{ \'sorting\': parse(column.sortable), \'sorting_asc\': params.sorting()[parse(column.sortable)]==\'asc\', \'sorting_desc\': params.sorting()[parse(column.sortable)]==\'desc\' }" ng-click="sortBy(column, $event)" ng-show="column.show(this)" ng-init="template=column.headerTemplateURL(this)" class="header {{column.class}}"> <div class="inline" ng-if="!template" ng-show="!template" ng-bind="parse(column.title)"></div> <div  class="inline" ng-if="template" ng-show="template"><div ng-include="template"></div></div> </th> </tr> <tr ng-show="show_filter" class="ng-table-filters"> <th ng-repeat="column in $columns" ng-show="column.show(this)" class="filter"> <div ng-repeat="(name, filter) in column.filter"> <div ng-if="column.filterTemplateURL" ng-show="column.filterTemplateURL"> <div ng-include="column.filterTemplateURL"></div> </div> <div ng-if="!column.filterTemplateURL" ng-show="!column.filterTemplateURL"> <div ng-include="\'ng-table/filters/\' + filter + \'.html\'"></div> </div> </div> </th> </tr>');
        $templateCache.put('ng-table/pager.html', '<div class="ng-cloak dataTables_paginate paging_bootstrap"><div ng-if="params.data.length == 0" class="width-100 text-center">Chưa có dữ liệu/(No Data)</div><div ng-if="params.settings().total > 10" class="pull-right"><select class="height-32px" ng-init="params.settings().count" ng-model="params.settings().count" ng-options="c for c in params.settings().counts" ng-change="params.count(params.settings().count)"></select></div><ul class="pagination"> <li ng-class="{ disabled : params.page() === 1 }"  ng-if="lastpage > 5" title="Về đầu trang" class="hidden-768">  <a ng-click="params.page(1)"><i class="icon-double-angle-left smaller-90"></i></a> </li> <li ng-class="{\'disabled\': !page.active, \'active\': params.page()===page.number && (page.type !==\'prev\' && page.type !==\'next\')}" ng-repeat="page in pages" ng-switch="page.type"> <a ng-switch-when="prev" ng-click="params.page(page.number)"><i class="fa fa-angle-left"></i></a><a ng-switch-when="first" ng-click="params.page(page.number)">{{page.number}}</a><a ng-switch-when="page" ng-click="params.page(page.number)">{{page.number}}</a><a ng-switch-when="more" ng-click="params.page(page.number)">&#8230;</a><a ng-switch-when="last" ng-click="params.page(page.number)">{{page.number}}</a><a ng-switch-when="next" ng-click="params.page(page.number)"><i class="fa fa-angle-right"></i></a></li> <li ng-class="{ disabled : params.page() === lastpage }"  ng-if="lastpage > 5" title="Về cuối trang" class="hidden-768" > <a ng-click="params.page(lastpage)"><i class="icon-double-angle-right smaller-90"></i></a> </li> <li class="disabled hidden-768"> <a ng-show="lastpage > 2">Trang </a><input type="text" style="width:42px; height:32px !important; float:left; border-radius:0px !important;margin:0" ng-show="lastpage > 2" ng-model="curPage" ng-change="changePage(curPage)" /><a ng-show="lastpage > 2"> / {{lastpage > 2 ? lastpage : 1}}</a></li></ul></div>');
    }]);
